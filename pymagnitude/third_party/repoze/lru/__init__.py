    ...
    def __call__(self, func):
        ...
        def cached_wrapper(*args, **kwargs):
            def list_to_tuple(list_of_lists):
                return tuple(
                    tuple(x) if isinstance(x, list) else x for x in list_of_lists
                )
            ...
            def get_default_args(remove_self, func, rargs, kwargs):
                ...
                if defaults is not None:
                    ...
                    new_kwargs = dict(
                        zip(args[-len(defaults) :], rargs[len(args) - len(defaults) :])
                    )
            ...
            try:
                args, kwargs = get_default_args(
                    self.remove_self, self.real_func or func, args, kwargs
                )
                args_t = tuple(
                    list_to_tuple(arg) if isinstance(arg, list) else arg for arg in args
                )
                kwargs_t = frozenset(
                    (
                        (kwarg[0], list_to_tuple(kwarg[1]))
