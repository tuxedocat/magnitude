...
def _log(*args):
    ...
    if not _log.disable_message:
        print(
            "[Magnitude] Magnitude is logging messages for slow "
...
class Magnitude(object):
    ...
    BOW = BOW
    EOW = EOW
    RARE_CHAR = "\uF002".encode("utf-8")
    FTS_SPECIAL = set("*^")
    MMAP_THREAD_LOCK = {}
    OOV_RNG_LOCK = threading.Lock()
    SQLITE_MAX_VARIABLE_NUMBER = max(
        max((_sqlite_try_max_variable_number(n) for n in [99, 999, 9999, 99999])), 1
    )
    ...
    def __init__(
        ...
        if self.stream:
            ...
            self.http_download_vfs = HTTPVFS(
                vfsname="http_download", options=download_vfs_options
            )
        else:
            self.driver = sqlite3
        ...
        if path is None:
            ...
            self.path = (
                ...
                if not self.stream
                else MagnitudeUtils.download_model(path, _download=False, _local=True)
            )
        self._all_conns = []
        ...
        if (
            self.path.endswith(".bin")
            or self.path.endswith(".txt")
            or self.path.endswith(".vec")
            or self.path.endswith(".hdf5")
        ):
            if not supress_warnings:
                ...
                sys.stdout.flush()
            from pymagnitude.converter_shared import (
                convert as convert_vector_file,
            )  # noqa

            self.path = convert_vector_file(self.path)

        # If the path doesn't exist locally, try a remote download
        if not self.stream and not os.path.isfile(self.path) and not self.memory_db:
        ...
        self.length = (
            self._db()
            .execute("SELECT value FROM magnitude_format WHERE key='size'")
            .fetchall()[0][0]
        )
        version_query = (
            self._db()
            .execute("SELECT value FROM magnitude_format WHERE key='version'")
            .fetchall()
        )
        self.version = version_query[0][0] if len(version_query) > 0 else 1
        elmo_query = (
            self._db()
            .execute("SELECT value FROM magnitude_format WHERE key='elmo'")
            .fetchall()
        )
        ...
        if CONVERTER_VERSION < self.version:
            raise RuntimeError(
                ...
                with this particular `.magnitude` file."""
            )  # noqa
        self.emb_dim = (
            self._db()
            .execute("SELECT value FROM magnitude_format WHERE key='dim'")
            .fetchall()[0][0]
        )
        self.precision = (
            self._db()
            .execute("SELECT value FROM magnitude_format WHERE key='precision'")
            .fetchall()[0][0]
        )
        subword_query = (
            self._db()
            .execute("SELECT value FROM magnitude_format WHERE key='subword'")
            .fetchall()
        )
        ...
        if self.subword:
            self.subword_start = (
                self._db()
                .execute("SELECT value FROM magnitude_format WHERE key='subword_start'")
                .fetchall()[0][0]
            )
            self.subword_end = (
                self._db()
                .execute("SELECT value FROM magnitude_format WHERE key='subword_end'")
                .fetchall()[0][0]
            )
        approx_query = (
            self._db()
            .execute("SELECT value FROM magnitude_format WHERE key='approx'")
            .fetchall()
        )
        ...
        if self.approx:
            self.approx_trees = (
                self._db()
                .execute("SELECT value FROM magnitude_format WHERE key='approx_trees'")
                .fetchall()[0][0]
            )
        self.dim = self.emb_dim + self.placeholders
        self.highest_entropy_dimensions = [
            row[0]
            for row in self._db()
            .execute("SELECT value FROM magnitude_format WHERE key='entropy'")
            .fetchall()
        ]
        duplicate_keys_query = (
            ...
            WHERE key='max_duplicate_keys'"""
            )
            .fetchall()
        )
        self.max_duplicate_keys = (
            len(duplicate_keys_query) > 0 and duplicate_keys_query[0][0]
        )
        if len(duplicate_keys_query) == 0:
            ...
            self.max_duplicate_keys = (
                duplicate_keys_query[0][0]
                if duplicate_keys_query[0][0] is not None
                else 1
            )  # noqa
        ...
        if self.eager:
            mmap_thread = threading.Thread(target=self.get_vectors_mmap, args=(False,))
    ...
    def _setup_for_mmap(self):
        ...
        self.path_to_mmap = os.path.join(self.temp_dir, self.md5 + ".magmmap")
        self.path_to_approx_mmap = os.path.join(
            self.temp_dir, self.md5 + ".approx.magmmap"
        )
        self.path_to_elmo_w_mmap = os.path.join(
            self.temp_dir, self.md5 + ".elmo.hdf5.magmmap"
        )
        self.path_to_elmo_o_mmap = os.path.join(
            self.temp_dir, self.md5 + ".elmo.json.magmmap"
        )
        ...
        if self.path_to_approx_mmap not in Magnitude.MMAP_THREAD_LOCK:
            Magnitude.MMAP_THREAD_LOCK[self.path_to_approx_mmap] = threading.Lock()
        if self.path_to_elmo_w_mmap not in Magnitude.MMAP_THREAD_LOCK:
            Magnitude.MMAP_THREAD_LOCK[self.path_to_elmo_w_mmap] = threading.Lock()
        if self.path_to_elmo_o_mmap not in Magnitude.MMAP_THREAD_LOCK:
            Magnitude.MMAP_THREAD_LOCK[self.path_to_elmo_o_mmap] = threading.Lock()
        ...
        self.APPROX_MMAP_THREAD_LOCK = Magnitude.MMAP_THREAD_LOCK[
            self.path_to_approx_mmap
        ]
        self.APPROX_MMAP_PROCESS_LOCK = InterProcessLock(
            self.path_to_approx_mmap + ".lock"
        )
        self.ELMO_W_MMAP_THREAD_LOCK = Magnitude.MMAP_THREAD_LOCK[
            self.path_to_elmo_w_mmap
        ]
        self.ELMO_W_MMAP_PROCESS_LOCK = InterProcessLock(
            self.path_to_elmo_w_mmap + ".lock"
        )
        self.ELMO_O_MMAP_THREAD_LOCK = Magnitude.MMAP_THREAD_LOCK[
            self.path_to_elmo_o_mmap
        ]
        self.ELMO_O_MMAP_PROCESS_LOCK = InterProcessLock(
            self.path_to_elmo_o_mmap + ".lock"
        )
    ...
    def _db(self, force_new=False, downloader=False):
        ...
        if not conn_exists or force_new:
            if self.fd:
                if os.name == "nt":
                    conn = self.sqlite3_connect(
                        downloader, self.path, check_same_thread=False
                    )
                else:
                    conn = self.sqlite3_connect(
                        downloader, "/dev/fd/%d" % self.fd, check_same_thread=False
                    )
            elif self.stream:
                conn = self.sqlite3_connect(
                    downloader, self.path, check_same_thread=False
    ...
    def _create_empty_db(self, db):
        ...
        collision_error_allowed = 0.001
        number_of_dims = max(
            math.ceil(
                math.log(
                    (
                        (self._number_of_values ** 2)
                        / (-2 * math.log(-collision_error_allowed + 1))
                    ),
                    100,
                )
            ),
    ...
    def _key_t(self, key):
        ...
        sensitivity.
        """
        if self.case_insensitive and (isinstance(key, str) or isinstance(key, unicode)):
            return key.lower()
        return key
    ...
    def _db_query_similar_keys_vector(self, key, orig_key, topn=3, normalized=None):
        ...
        def _sql_escape_fts(s):
            return "".join(
                "\\" + c if c in Magnitude.FTS_SPECIAL else c for c in s
            ).replace('"', '""')
        ...
        if self.subword and len(key) < Magnitude.MAX_KEY_LENGTH_FOR_OOV_SIM:
                ...
                for exact_match in exact_matches:
                    ...
                    for i, e in enumerate(exact_match):
                        ...
                        split_results.extend(
                            self._db()
                            .execute(exact_search_query, (e, e, limit))
                            .fetchall()
                        )
                        results.extend(
                            self._db()
                            .execute(exact_search_query, (e, e, topn))
                            .fetchall()
                        )
            ...
            if len(results) == 0:
                ...
                while (
                    len(results) < topn and current_subword_start >= self.subword_start
                ):
                    ngrams = list(
                        char_ngrams(key, current_subword_start, current_subword_start)
                    )
                    ngram_limit_map = {
                        6: 4,
                        5: 8,
                    ...
                    while (
                        current_subword_start in ngram_limit_map
                        and len(ngrams) > ngram_limit_map[current_subword_start]
                    ):
                        # Reduce the search parameter space by sampling every
                        # other ngram
                        ngrams = ngrams[:-1][::2] + ngrams[-1:]
                    params = (
                        " OR ".join('"{0}"'.format(_sql_escape_fts(n)) for n in ngrams),
                        topn,
                    )
                    results = self._db().execute(search_query, params).fetchall()
                    small_typo = (
                        len(results) > 0
                        and self._string_dist(results[0][0].lower(), orig_key.lower())
                        <= 4
                    )
                    if (
                        key_shrunk_stemmed != orig_key
                        and key_shrunk_stemmed != key_shrunk
                        and not small_typo
                    ):  # noqa
                        ngrams = list(
                            char_ngrams(
                        ...
                        params = (
                            " OR ".join(
                                '"{0}"'.format(_sql_escape_fts(n)) for n in ngrams
                            ),
                            topn,
                        )
                        results = self._db().execute(search_query, params).fetchall()
            ...
            results = (
                self._db()
                .execute(exact_search_query, (orig_key, orig_key, topn))
                .fetchall()
            )
        final_results = []
        for result in results:
            result_key, vec = self._db_full_result_to_vec(result, normalized=normalized)
    ...
    def _process_lm_output(self, q, normalized):
        ...
        if self.elmo:
            if zero_d:
                r_val = np.concatenate(
                    self.get_elmo_embedder().embed_batch([[q]])[0], axis=1
                ).flatten()
            elif one_d:
                r_val = np.concatenate(
                    self.get_elmo_embedder().embed_batch([q])[0], axis=1
        ...
        if self.placeholders > 0 or self.ngram_oov:
            shape_p = (
                list(r_val.shape)
                if zero_d or one_d
                else ([len(r_val)] + list(max((row.shape for row in r_val))))
            )
            shape_p[-1] = self.dim
            if self.placeholders > 0:
                if zero_d or one_d:
                    ...
                    r_val_p = [np.zeros(shape_p[1:], dtype=self.dtype) for row in r_val]
            else:
                r_val_p = r_val
            if self.ngram_oov:
                if zero_d:
            ...
            for idx in product(*[xrange(s) for s in shape_p[:-1]]):
                if zero_d:
                    ...
                    key = q[idx[0]]
                    if self.ngram_oov:
                        vec = r_val[idx] if self.__contains__(key) else lookup[idx[0]]
                    else:
                        vec = r_val[idx]
    ...
    def _out_of_vocab_vector(self, key, normalized=None, force=False):
        ...
        if not is_str:
            ...
            ngrams = char_ngrams(key, Magnitude.NGRAM_BEG, Magnitude.NGRAM_END)
            random_vectors = []
        ...
        if is_str:
            ...
            final_vector = (
                ...
                + self._db_query_similar_keys_vector(
                    key, orig_key, normalized=normalized
                )
                * 0.7
            )
            if normalized:
    ...
    def _db_batch_generator(self, params):
        ...
        if len(params) <= Magnitude.SQLITE_MAX_VARIABLE_NUMBER:
            ...
            it = iter(params)
            for batch in iter(
                lambda: tuple(islice(it, Magnitude.SQLITE_MAX_VARIABLE_NUMBER)), ()
            ):
                yield batch
    ...
    def _db_result_to_vec(self, result, normalized=None):
        ...
        if self.use_numpy:
            ...
            if normalized:
                rv = [
                    v / float(10 ** self.precision)
                    for v in islice(result, self.emb_dim)
                ] + [0.0] * self.placeholders
            else:
                rv = [
                    v * (float(result[-1]) / float(10 ** self.precision))
                    for v in islice(result, self.emb_dim)
                ] + [0.0] * self.placeholders
        return rv
    ...
    def _vectors_for_keys_cached(self, keys, normalized=None, force=False):
        ...
        if len(unseen_keys) > 0:
            unseen_keys_map = {self._key_t(k): i for i, k in enumerate(unseen_keys)}
            ...
            for unseen_keys_batch in self._db_batch_generator(unseen_keys):
                ...
                for result in results:
                    ...
                    if result_key_t in unseen_keys_map:
                        ...
                        ) and (self.case_insensitive or result_key == unseen_keys[i]):
            ...
            for i in range(len(unseen_vectors)):
                self._vector_for_key_cached._cache.put(
                    (
                        (unseen_keys[i],),
                        frozenset([("normalized", normalized)]),
                    ),  # noqa
                    unseen_vectors[i],
                )
        ...
        vectors = [
            self.query(key, normalized=normalized)
            if key not in unseen_keys_map
            else unseen_vectors[unseen_keys_map[self._key_t(key)]]
            for key in keys
        ]
        return vectors
    ...
    def _vectors_for_2d_keys(self, keys2d, normalized=None):
        ...
        if self._is_lm():
            ...
            return (self._vectors_for_keys_cached(row, normalized) for row in keys2d)
    ...
    def _keys_for_indices(self, indices, return_vector=True):
        ...
        unseen_indices = tuple(
            ...
            if self._key_for_index_cached._cache.get(
                ((index,), frozenset([("return_vector", return_vector)]))  # noqa
            )
            is None
        )  # noqa
        unseen_indices_map = {}
        if len(unseen_indices) > 0:
            ...
            if return_vector:
                columns = "*"
            unseen_indices_map = {
                (index - 1): i for i, index in enumerate(unseen_indices)
            }
            unseen_keys = [None] * len(unseen_indices)
            for unseen_indices_batch in self._db_batch_generator(unseen_indices):
                results = self._db().execute(
                    """
                        SELECT rowid, """
                    + columns
            ...
            for i in range(len(unseen_keys)):
                if unseen_keys[i] is None:
                    raise IndexError(
                        "The index %d is out-of-range" % unseen_indices[i] - 1
                    )
        keys = [
            self.index(index, return_vector=return_vector)
            if index not in unseen_indices_map
            else unseen_keys[unseen_indices_map[index]]
            for index in indices
        ]
        return keys
    ...
    def query(
        ...
        if not isinstance(q, list):  # Single key
            ...
            if vec is None:
                ...
                return vec
        elif isinstance(q, list) and (
            len(q) == 0 or not isinstance(q[0], list)
        ):  # 1D list
            ...
            if self.use_numpy:
                tensor = np.zeros((len(q), pad_to_length, self.dim), dtype=self.dtype)
            else:
                tensor = [
                    [self._padding_vector() for i in range(pad_to_length)]
                    for j in range(len(q))
                ]
            for row, vectors in enumerate(self._vectors_for_2d_keys(q, normalized)):
    ...
    def _query_numpy(self, key, contextualize=False, normalized=None):
        ...
        key_0_0_is_number = (
            key_0_is_list and key_0_len_ge_0 and isinstance(key[0][0], Number)
        )
        r_val = None
        if (
            key_is_ndarray or key_0_is_number or key_0_is_ndarray or key_0_0_is_number
        ):  # noqa
            r_val = key
    ...
    def _query_is_cached(self, key, normalized=None):
        ...
        normalized = normalized if normalized is not None else self.normalized
        return (
            self._vector_for_key_cached._cache.get(
                (key, frozenset([("normalized", normalized)]))
            )
            is not None
        ) or (  # noqa
    ...
    def distance(self, key, q):
        ...
        if not isinstance(q, list):
            ...
            return [
                np.linalg.norm(a - b)
                for b in self._query_numpy(
                    q, contextualize=True, normalized=self.normalized
                )
            ]
    ...
    def similarity(self, key, q):
        ...
        if not isinstance(q, list):
            ...
            return [
                np.inner(a, b) / (np.linalg.norm(a) * np.linalg.norm(b))
    ...
    def doesnt_match(self, q):
        ...
        """
        mean_vector = np.mean(
            self._query_numpy(q, contextualize=True, normalized=True), axis=0
        )
        mean_unit_vector = mean_vector / np.linalg.norm(mean_vector)
        distances = [
            np.linalg.norm(mean_unit_vector - b)
            for b in self._query_numpy(q, contextualize=True, normalized=True)
        ]
    ...
    def _db_query_similarity(
        ...
        exclude_keys = {self._key_t(exclude_key) for exclude_key in exclude_keys}
        ...
        if (DISTANCE or APPROX) and (len(negative) > 0 or len(positive) > 1):
            positive_vecs = np.sum(
                self._query_numpy(positive, contextualize=True, normalized=True), axis=0
            )
            if len(negative) > 0:
                negative_vecs = -1.0 * np.sum(
                    self._query_numpy(negative, contextualize=True, normalized=True),
                    axis=0,
                )
            else:
                negative_vecs = np.zeros((self.dim,), dtype=self.dtype)
            mean_vector = (positive_vecs + negative_vecs) / float(
                len(positive) + len(negative)
            )
            mean_unit_vector = mean_vector / np.linalg.norm(mean_vector)
        elif DISTANCE or APPROX:
            mean_unit_vector = self._query_numpy(positive[0], normalized=True)
        elif COSMUL:
        ...
        if DISTANCE or COSMUL:
            filtered_indices = []
            for batch_start, _, batch in self.get_vectors_mmap_batch_generator():
                if DISTANCE:
                    similiarities = np.dot(batch, mean_unit_vector)
                elif COSMUL:
                    positive_similiarities = [
                        ((1 + np.dot(batch, vec)) / 2) for vec in positive_vecs
                    ]
                    negative_similiarities = [
                        ((1 + np.dot(batch, vec)) / 2) for vec in negative_vecs
                    ]
                    similiarities = np.prod(positive_similiarities, axis=0) / (
                        np.prod(negative_similiarities, axis=0) + 0.000001
                    )

                partition_results = np.argpartition(
                    similiarities, -1 * min(filter_topn, self.batch_size, self.length)
                )[-filter_topn:]

                for index in partition_results:
                    if min_similarity is None or similiarities[index] >= min_similarity:
                        if len(filtered_indices) < filter_topn:
                            heapq.heappush(
                                filtered_indices,
                                (similiarities[index], batch_start + index),
                            )
            ...
            topn_indices = heapq.nlargest(
                filter_topn, filtered_indices, key=lambda x: x[0]
            )
            ...
            nns = approx_index.get_nns_by_vector(
                mean_unit_vector, filter_topn, search_k=search_k, include_distances=True
            )
            topn_indices = izip(nns[1], nns[0])
            topn_indices = imap(lambda di: (1 - di[0] ** 2 * 0.5, di[1]), topn_indices)

        # Tee topn_indices iterator
        topn_indices_1, topn_indices_2 = tee(topn_indices)

        # Retrieve the keys of the vectors
        keys = self.index([i[1] for i in topn_indices_1], return_vector=False)
    ...
    def _handle_pos_neg_args(self, positive, negative):
        if not isinstance(positive, list) or (
            len(positive) > 0 and isinstance(positive[0], Number)
        ):
            positive = [positive]
        if not isinstance(negative, list) or (
            len(negative) > 0 and isinstance(negative[0], Number)
        ):
            negative = [negative]
    ...
    def _exclude_set(self, positive, negative):
        def _is_vec(elem):
            return isinstance(elem, np.ndarray) or (
                isinstance(elem, list) and len(elem) > 0 and isinstance(elem[0], Number)
            )

        return frozenset(
            (
                elem
                for elem in chain.from_iterable([positive, negative])
                if not _is_vec(elem)
            )
        )
    ...
    def most_similar_approx(
        ...
        if not self.approx:
            raise RuntimeError(
                "The `.magnitude` file you are using does not \
...
build the appropriate indexes into the `.magnitude` file."
    ...
    def closer_than(self, key, q, topn=None):
        ...
        return self.most_similar(
            key, topn=topn, min_similarity=min_similarity, return_similarities=False
        )
                ...
                try:
                    ...
                    if tlock and plock:
                        values = imap(
                            lambda kv: kv[1],
                            self._iter(
                                put_cache=self.lazy_loading == -1, downloader=True
                            ),
                        )
                        try:
                            with open(path_to_mmap_temp, "w+b") as mmap_file:
                                ...
                                for i, value in enumerate(values):
                                    progress = round(
                                        (float(i) / float(self.length)) * 100, 2
                                    )  # noqa
                                    if (
                                        log and self.log and int(progress) > last_p
                                    ):  # noqa
                                        last_p = int(progress)
                                        _log("Progress: %.2f%%" % (progress,))
                                    all_vectors[i] = value
                                all_vectors.flush()
                                try:
                                    del all_vectors
    ...
    def get_vectors_mmap_batch_generator(self):
        ...
        if self.length > self.batch_size:
            for i in range(all_vectors.shape[0]):
                ...
                batch_end = min(batch_start + self.batch_size, all_vectors.shape[0])
                if batch_start >= all_vectors.shape[0]:
                    break
                yield (batch_start, batch_end, all_vectors[batch_start:batch_end])
        ...
        if self._approx_index is None:
            ...
            while True:
                ...
                try:
                    ...
                    if not logged and log and self.log:
                        _log(
                            "Need to build the approximate index."
                            " This may take some time...but it only "
                            "needs to be done once (even between "
                            "multiple runs of this program). The result"
                            " will get stashed into a temporary "
        ...
        if self._elmo_embedder is None:
            ...
            while True:
                ...
                try:
                    ...
                    path_to_elmo_w_mmap_temp = self.path_to_elmo_w_mmap + ".tmp"
                    path_to_elmo_o_mmap_temp = self.path_to_elmo_o_mmap + ".tmp"
                    ...
                    if tlock_w and plock_w and tlock_o and plock_o:
                        try:
                            with open(path_to_elmo_w_mmap_temp, "w+b") as mmap_file:
                                last_p = 0
                                for i, (length, chunk) in enumerate(meta_1_chunks):
                                    progress = round(
                                        (float(i) / float(length)) * 100, 2
                                    )  # noqa
                                    if (
                            ...
                            if not self.closed:
                                ...
                                return
                            with open(path_to_elmo_o_mmap_temp, "w+b") as mmap_file:
    ...
    def __getitem__(self, q):
        ...
        if isinstance(q, slice):
            return self.index(list(range(*q.indices(self.length))), return_vector=True)
        else:
    ...
    def close(self):
        ...
        try:
            ...
            pass
        if (
            hasattr(self, "MMAP_PROCESS_LOCK")
            and hasattr(self.MMAP_PROCESS_LOCK, "lockfile")
            and self.MMAP_PROCESS_LOCK.lockfile is not None
        ):
            try:
                ...
                pass
        if (
            hasattr(self, "APPROX_MMAP_PROCESS_LOCK")
            and hasattr(self.APPROX_MMAP_PROCESS_LOCK, "lockfile")
            and self.APPROX_MMAP_PROCESS_LOCK.lockfile is not None
        ):
            try:
                ...
                pass
        if (
            hasattr(self, "ELMO_W_MMAP_PROCESS_LOCK")
            and hasattr(self.ELMO_W_MMAP_PROCESS_LOCK, "lockfile")
            and self.ELMO_W_MMAP_PROCESS_LOCK.lockfile is not None
        ):
            try:
                ...
                pass
        if (
            hasattr(self, "ELMO_O_MMAP_PROCESS_LOCK")
            and hasattr(self.ELMO_O_MMAP_PROCESS_LOCK, "lockfile")
            and self.ELMO_O_MMAP_PROCESS_LOCK.lockfile is not None
        ):
            try:
...
class FeaturizerMagnitude(Magnitude):
    ...
    def __init__(self, number_of_values=1000000, namespace=None, **kwargs):
        ...
        super(FeaturizerMagnitude, self).__init__(
            None,
...
class ConcatenatedMagnitude(object):
    ...
    def _dstack(self, l, use_numpy):
        ...
        if use_numpy:
            ...
            return [
                self._hstack((l3[example] for l3 in l), use_numpy=use_numpy)
...
class MagnitudeUtils(object):
    ...
    def download_model(
        ...
        if not os.path.isfile(os.path.join(download_dir, local_file_name)):
            try:
                ...
                urlretrieve(
                    remote_file_path, os.path.join(download_dir, local_file_name_tmp)
                )
                ...
                os.rename(
                    ...
                    os.path.join(download_dir, local_file_name),
                )
            except BaseException:
                if _local:
                    raise RuntimeError(
                        "The path to the Magnitude file at '"
                        + orig_model
                        + "' could not be found. Also failed to find a valid remote model at the following URL: "
                        + remote_file_path  # noqa
                    )
                else:
                    raise RuntimeError(
                        "The download could not be completed. Are you sure a valid model exists at the following URL: "
                        + remote_file_path  # noqa
                    )
    ...
    def batchify(X, y, batch_size):  # noqa: N803
        ...
        X_batch_generator = cycle(
            [X[i : i + batch_size] for i in xrange(0, len(X), batch_size)]  # noqa: N806
        )
        y_batch_generator = cycle(
            [y[i : i + batch_size] for i in xrange(0, len(y), batch_size)]
        )
    ...
    def class_encoding():
        ...
        def add_class(c):
            ...
            int_to_class_map = None
            return class_to_int_map.setdefault(c, len(class_to_int_map))
        ...
        def int_to_class(i):
            ...
            if int_to_class_map is None:
                int_to_class_map = {
                    ...
                    for k, v in (
                        (
                            hasattr(class_to_int_map, "iteritems")
                            and class_to_int_map.iteritems  # noqa
                        )
                        or class_to_int_map.items
                    )()
                }
...
if _APSW_LIB == "internal":
    ...
    class HTTPVFSFileCache:
            ...
            for c in self.vfsfile._get_caches():
                ...
                if start >= 0 and c.length_of_data() >= end:
                    ...
                    if start >= c.running_hit_last_end:
                        ...
                        if (
                            ...
                            c.running_hit_direction = max(
                                ...
                                (c.running_backward_hit_amount, -1),
                            )[1]
                        else:
                    ...
                    c.running_hit_amount = max(
                        c.running_forward_hit_amount, c.running_backward_hit_amount
                    )
                    ...
                    (
                        return_val[0] is None
                        or (
                            isinstance(return_val, list)
                            and c.running_hit_amount > return_val[0]
                        )
                    )
                    and start >= c.running_hit_last_end  # noqa
                    and close_to_last_end
                ):
                    ...
                    and end <= c.running_hit_last_start  # noqa
                    and close_to_last_start
                ):
        ...
        def write_data(self, start_offset, data, amount, offset):
            ...
            if self.vfsfile.should_cache:
                ...
                current_time = time.time()
                if (
                    current_time - self.vfsfile.last_cache_purge
            ...
            return data[offset - start_offset : (offset - start_offset) + amount]

        def _prefetch_in_background(
            self, _prefetch_in_background, amount, offset, sequential
        ):
            """Prefetches data from the network to the cache."""
            # Store the extra data fetched back in the network cache
            if self.vfsfile.trace_log:
                print(
                    "[HTTPVFS] Prefetching in background @ %d + %d" % (offset, amount)
                )
            try:
                if sequential:
                ...
                if self.vfsfile.trace_log:
                    print("[HTTPVFS] Prefetching error @ %d + %d" % (offset, amount))
                pass

        def prefetch_in_background(
            self, _prefetch_in_background, amount, offset, sequential=False
        ):
            """Prefetches data from the network to the cache
            in the background."""
            if self.vfsfile.trace_log:
            ...
            if (
                len(self.vfsfile.prefetch_threads) <= self.vfsfile.prefetch_thread_limit
                or sequential
            ):
                prefetch_thread = threading.Thread(
                    target=self._prefetch_in_background,
                    args=(_prefetch_in_background, amount, offset, sequential),
                ...
                if self.vfsfile.trace_log:
                    print(
                        ...
                        "reached prefetch thread limit" % (offset, amount)
                    )
        ...
        def read_data(self, amount, offset, _prefetch_in_background=None):
            ...
            if running_hit_amount is not None:
                if (
                    self.vfsfile.sequential_cache_exponential_read_growth
                    and cache is None
                ):
                    ...
                    running_hit_amount = min(
                        running_hit_amount * 2, self.vfsfile.sequential_cache_max_read
                    )
                ...
                if cache is None:
                    self.vfsfile.cache_amount = min(
                        running_hit_amount, self.vfsfile.sequential_cache_max_read
                    )
                    self.save_cache()
            else:
                if cache is None:
            ...
            if cache:
                ...
                self.vfsfile.cache_amount = min(
                    cache.running_hit_amount, self.vfsfile.sequential_cache_max_read
                )
                ...
                if (
                    current_time - self.vfsfile.last_random_access_hit_tracker_purge
                ) > self.vfsfile.ttl_purge_interval:  # noqa
                    if self.vfsfile.trace_log:
                        print("[HTTPVFS] Purging expired hit trackers...")
                    self.vfsfile.last_random_access_hit_tracker_purge = current_time
                    self.vfsfile.hit_pattern = [
                        hit
                        for hit in self.vfsfile.hit_pattern
                        if (
                            (current_time - hit[4])
                            <= self.vfsfile.random_access_hit_tracker_ttl
                        )
                    ]  # noqa

                # Find the closest cluster of hits for the current miss
                hit_index = bisect.bisect_left(
                    KeyList(self.vfsfile.hit_pattern, key=lambda x: x[0]), offset
                )
                hit_index_area = []
                if hit_index - 1 >= 0:
                ...
                if len(hit_index_area) > 0:
                    hit_index = min(
                        hit_index_area,
                        key=lambda x: abs(self.vfsfile.hit_pattern[x][0] - offset),
                    )
    ...
    class HTTPVFSFileMemoryMappedCache(HTTPVFSFileCache):
        ...
        def __init__(self, vfsfile, cache_dir_path, cache_key=None):
            ...
            if self.cache_key and self.cache_key != ".DS_Store":
                ...
                self.running_hit_last_start = (
                    float(cache_key_split[2])
                    if cache_key_split[2] == "inf"
                    else int(cache_key_split[2])
                )
        ...
        def length_of_data(self):
            ...
            try:
                return os.path.getsize(
                    os.path.join(self.cache_dir_path, self.cache_key)
                )
            except BaseException:
                return 0
        ...
        def add_to_mmaps(self, new, mm):
            ...
            reached."""
            while len(self.vfsfile.cache_mmaps_heap) >= self.vfsfile.mmap_max_files:
                ...
                try:
                    ...
                    pass
            heapq.heappush(self.vfsfile.cache_mmaps_heap, (time.time(), new))
        ...
        def get_mmap(self, create=True):
            ...
            if self.cache_key not in self.vfsfile.cache_mmaps and create:
                joined = os.path.join(self.cache_dir_path, self.cache_key)
                if os.path.exists(os.path.join(self.cache_dir_path, self.cache_key)):
                    f = open(joined, "r+b")
        ...
        def set_data(self, data):
            ...
            try:
                ...
                pass
            f = open(os.path.join(self.cache_dir_path, self.cache_key), "w+b")
            f.write(data)
            f.flush()
            os.fsync(f.fileno())
            mm = None
        ...
        def save_cache(self):
            ...
            old = os.path.join(self.cache_dir_path, self.cache_key)
            ...
            for cache in self.vfsfile._get_caches():
                ...
                if (current_time - cache.time) > self.vfsfile.cache_ttl:
                    ...
                    try:
                        ...
                        pass
                    try:
                        os.remove(os.path.join(cache.cache_dir_path, cache.cache_key))
                    except BaseException:
                        pass
    ...
    class HTTPVFSFile(apsw.VFSFile):
        ...
        def __init__(self, inheritfromvfsname, name, flags, vfs, options=None):
            ...
            self.cache_dir = (
                hashlib.md5(self.url.encode("utf-8")).hexdigest() + "_supersqlmmap"
            )
        ...
        def _new_connection(self):
            ...
            if self.parsed_url.scheme.lower() == "http":
                return http.client.HTTPConnection(self.parsed_url.netloc, timeout=60)
            else:
                return http.client.HTTPSConnection(self.parsed_url.netloc, timeout=60)
        ...
        def _network_error(self, e, i):
            ...
            if i + 1 >= self.tries:
                raise RuntimeError("Could not reach the server at: '" + self.url + "'")
            else:
                if self.trace_log:
        ...
        def _prefetch_in_background(self, n, amount, offset):
            headers = {
                ...
                + str(max(offset, 0))
                + "-"
                + str(min((offset + amount) - 1, self.length)),  # noqa
            }

            self._wait_on_prefetch_connection(n)
            while not self.pconn_terminated[n]:
                try:
                    self.pconn[n].request("GET", self.parsed_url.path, headers=headers)
                    break
        ...
        def _get_caches(self):
            ...
            if self.use_mmap:
                return [
                    HTTPVFSFileMemoryMappedCache(self, self.cache_dir_path, cache_key)
                    for cache_key in os.listdir(self.cache_dir_path)
                ]
        ...
        def xRead(self, amount, offset):  # noqa: N802
            ...
            for i in range(self.tries):
                try:
                    ...
                    # and cached it
                    if self.use_mmap:
                        cache = HTTPVFSFileMemoryMappedCache(self, self.cache_dir_path)
                    else:
                        cache = HTTPVFSFileCache(self)
                    data = cache.read_data(amount, offset, self._prefetch_in_background)
                    if data is None:
                        ...
                        self.conn_lock.acquire()
                        self.conn.request("GET", self.parsed_url.path, headers=headers)
                        res = self.conn.getresponse()
                        if not (res.status >= 200 and res.status <= 299):
                            # Check for a valid status from the server
                            raise RuntimeError("HTTP Status Code Error from Server")
                        if self.trace_log:
                            print(
            ...
            with self.vfs.files_lock:
                if ident in self.vfs.files:
                    if self.vfs.files[ident][0] <= 1:
                        ...
                        if self.sequential_prefetch_thread:
                            self.sequential_prefetch_thread.do_run = False
                        self._prepare_prefetch_connection(self.RANDOM_ACCESS, new=False)
                        self._prepare_prefetch_connection(self.SEQUENTIAL, new=False)
            ...
            if flags[0] & apsw.SQLITE_OPEN_MAIN_DB:
                ...
                with self.files_lock:
                    if ident not in self.files:
                        self.files[ident] = (
                            1,
                            HTTPVFSFile(self.basevfs, name, flags, self, self.options),
                        )
                    else:
                        self.files[ident] = (
                            self.files[ident][0] + 1,
                            self.files[ident][1],
                        )
